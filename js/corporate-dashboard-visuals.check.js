(function () {
  ...
  function renderCustomHtml(host, template, context) {
    if (!host) return;
    const html = applyTemplate(template, context?.model || {});
    host.innerHTML = html;
    executeInlineScripts(host, context || {});
  }
  ...
  function ensureCardCss() {
    ...
    .dehc-shell{position:relative;z-index:1;padding:18px;display:grid;grid-template-columns:320px 1fr;gap:16px}
    ...
    .dehc-gauge-wrap{position:relative;height:172px;margin-top:8px}.dehc-gauge{width:100%;height:100%}
    .dehc-score-core{position:absolute;inset:50% auto auto 50%;transform:translate(-50%,-38%);text-align:center;pointer-events:none}
    .dehc-score{font-size:54px;line-height:.9;font-weight:900;letter-spacing:-.04em}.dehc-score-label{margin-top:7px;font-size:12px;text-transform:uppercase;letter-spacing:.18em;color:rgba(23,23,119,.54);font-weight:800}
    ...
    .dehc-mini-label{font-size:11px;text-transform:uppercase;letter-spacing:.12em;color:rgba(23,23,119,.52);font-weight:800}.dehc-mini-value{margin-top:6px;font-size:22px;font-weight:900;line-height:1}.dehc-mini-sub{margin-top:6px;font-size:11px;color:rgba(23,23,119,.64)}
    ...
    .dehc-kpi-label{font-size:11px;letter-spacing:.12em;text-transform:uppercase;color:rgba(23,23,119,.52);font-weight:800}.dehc-kpi-value{margin-top:10px;font-size:31px;font-weight:900;line-height:.95}.dehc-kpi-sub{margin-top:7px;font-size:12px;color:rgba(23,23,119,.66)}
    ...
    .dehc-trend{width:100%;height:122px}.dehc-detail-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px 14px}.dehc-detail{padding:10px 10px 8px;border-radius:16px;background:rgba(255,255,255,.42);border:1px solid rgba(23,23,119,.05)}
  ...
  function renderExecHealthCard(target, data) {
        ...
        <div class="dehc-shell">
          <div class="dehc-panel dehc-hero">
            ...
            <div class="dehc-hero-footer">
              <div class="dehc-mini"><div class="dehc-mini-label">Top wait</div><div class="dehc-mini-value">${htmlEncode(data?.top_wait_type || '—')}</div><div class="dehc-mini-sub">${formatInt(data?.top_wait_count || 0)} session(s)</div></div>
    ...
    trend.setOption({
      ...
      xAxis: { type: 'category', boundaryGap: false, data: history.map((_, i) => i + 1), axisLine: { show: false }, axisTick: { show: false }, axisLabel: { show: false } },
      yAxis: { type: 'value', min: Math.max(0, Math.min.apply(null, history) - 10), max: Math.min(100, Math.max.apply(null, history) + 10), axisLine: { show: false }, axisTick: { show: false }, splitLine: { lineStyle: { color: 'rgba(23,23,119,.06)' } }, axisLabel: { show: false } },
      tooltip: { trigger: 'axis', backgroundColor: 'rgba(23,23,119,.96)', borderColor: 'rgba(8,8,238,.12)', textStyle: { color: '#fff' }, formatter: p => `Health score: <b>${p[0].data}</b>` },
      series: [{ type: 'line', smooth: true, symbol: 'none', data: history, lineStyle: { width: 3, color: meta.accent }, areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: meta.glow }, { offset: 1, color: 'rgba(255,255,255,0)' }]) } }]
    });
  }
